...
export default function ConversationsPage() {
  ...
  const view = async (id) => { try { const d = await getConversation(id); setSelected(d.conversation); setMessages(d.conversation.messages||[]); } catch{} };
            ...
            {loading ? <p style={{padding:20,color:'var(--text-secondary)'}}>Loading…</p> : conversations.length===0 ? <p style={{padding:20,color:'var(--text-secondary)'}}>No conversations yet.</p> :
              conversations.map(c=>(
                <div key={c.id} onClick={()=>view(c.id)} style={{padding:'14px 20px',borderBottom:'1px solid var(--border)',cursor:'pointer',background:selected?.id===c.id?'rgba(108,92,231,.1)':'transparent',transition:'background .15s'}}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}><span style={{fontWeight:500,fontSize:13}}>{c.agent?.name||'Agent'}</span><span style={{fontSize:12,color:'var(--text-secondary)'}}>{new Date(c.updatedAt).toLocaleDateString()}</span></div>
                  <div style={{fontSize:13,color:'var(--text-secondary)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{c.messages?.[0]?.content||'No messages'}</div>
